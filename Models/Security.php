...
class AuthDB
{
    ...
    function __construct(private PDO $pdo)
    {
        ...
        $this->statementReadUser = $pdo->prepare('SELECT * FROM `user` INNER JOIN adress ON adress.idUser = user.idUser WHERE user.idUser =:idUser');
    ...
    function isLoggedin(): array | false
    {
        ...
        if ($sessionId && $signature) {
            ...
            if (hash_equals($hash, $signature)) {
                ...
                if ($session) {
                    $this->statementReadUser->bindValue(':idUser', $session['userid']);
